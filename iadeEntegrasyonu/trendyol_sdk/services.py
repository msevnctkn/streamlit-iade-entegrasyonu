...
class BaseService:
    ...
    def __init__(self, api: 'api.TrendyolApiClient'):
        ...
        if self._api.is_test:
            ...
            self.base_url2 = "https://api.trendyol.com/"
...
class ReturnedOrdersIntegrationService(BaseService):
    ...
    def get_shipment_packages(self, filter_params, supplier_id=None):
        ...
        if supplier_id:
            endpoint = "suppliers/{}/claims".format(supplier_id)
        else:
            endpoint = "suppliers/{}/claims".format(self._api.supplier_id)
        if not filter_params:
            filter_params = {}
        params = {
                  "claimIds": filter_params.get("claimIds", None),
                  "claimItemStatus": filter_params.get("claimItemStatus", None),
                  "endDate": filter_params.get("endDate", None),
                  "startDate": filter_params.get("startDate", None),
                  "orderNumber": filter_params.get("orderNumber", None),
                  "size": filter_params.get("size", None),
    ...
    def approve_claim_line_items(self, claimId, supplier_id = None):

        if supplier_id:
            endpoint = "claims/{claimId}/items/approve".format(claimId)
                
        else:
            endpoint = "claims/{claimId}/items/approve".format(claimId)
        url = urljoin(self.base_url, endpoint)
      
        data = self._api.call("PUT", url, headers=None, files=None)
        return data
...
class AccountingAndFinanceIntegrationService(BaseService):
    ...
    def filter_with_date_validation_constraint(self, supplier_id=None, filter_params=None):
        if supplier_id:
            endpoint = "integration/finance/che/sellers/{}/settlements".format(supplier_id)
        else:
            endpoint = "integration/finance/che/sellers/{}/settlements".format(self._api.supplier_id)
        if not filter_params:
            filter_params = {}
        params = {
            "transactionType": filter_params.get("transactionType", None),
            "startDate": filter_params.get("startDate", None),
            "endDate": filter_params.get("endDate", None),
            "page": filter_params.get("page", None),
            "size": filter_params.get("size", None),
        ...
        url = urljoin(self.base_url2, endpoint)
        data = self._api.call("GET", url, params=params, headers=None, files=None)
        return data
    ...
    def get_other_financials(self, supplier_id=None, filter_params=None):
        if supplier_id:
            endpoint = "integration/finance/che/sellers/{}/otherfinancials".format(supplier_id,)
        else:
            endpoint = "integration/finance/che/sellers/{}/otherfinancials".format(self._api.supplier_id)
        if not filter_params:
            filter_params = {}
        params = {
            "transactionType": filter_params.get("transactionType", None),
    ...
    def coming_cargo_invoice(self, invoiceSerialNumber= None, supplier_id = None, ):
        if supplier_id:
            endpoint = "integration/finance/che/sellers/{}/cargo-invoice/{}/items".format(supplier_id, invoiceSerialNumber)
        else:
            endpoint = "integration/finance/che/sellers/{}/cargo-invoice/{}/items".format(self._api.supplier_id, invoiceSerialNumber)

        url = urljoin(self.base_url2, endpoint)
        data = self._api.call("GET", url, headers=None, files=None)
        return data
